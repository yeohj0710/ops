...
// ── API ─────────────────────────────────────────────────────────────────────────
async function api(params, { timeoutMs = 30000 } = {}) {
  ...
  try {
    ...
    try {
      ...
      throw new Error(`API 연결 실패 (${why}). 주소 ${API}, 인터넷과 사이트 상태를 본다`);
    }
...
async function getBalance() {
  const j = await api({ action: "balance" });
...
async function addOrder(service, link, quantity) {
  const j = await api({ action: "add", service: String(service), link, quantity: String(quantity) }, { timeoutMs: 45000 });
...
async function getStatuses(ids) {
  ...
  for (let i = 0; i < ids.length; i += 100) {
    ...
    const j = await api({ action: "status", orders: chunk.join(",") });
...
function spawnRun(id) {
  ...
  const child = spawn(process.execPath, [SELF, "run", id], {
    detached: true,
...
async function runLoop(id) {
  ...
  if (s.pid && s.pid !== process.pid && pidAlive(s.pid)) {
  ...
  while (true) {
    ...
    try {
      const orderId = await addOrder(s.service, s.link, s.qty);
      consecutiveErrors = 0;
      s = loadState(id);
...
async function cmdStatus() {
  ...
  if (has("api") && s.orders.length) {
    ...
    const st = await getStatuses(ids);
    const byStatus = {};
    let charged = 0;
...
const commands = { check: cmdCheck, plan: cmdPlan, start: cmdStart, run: () => runLoop(positional[0] || fail("run 에는 묶음 id 가 필요하다")), status: cmdStatus, stop: cmdStop, resume: cmdResume, list: cmdList, help };
if (!commands[cmd]) {
  help();
  process.exitCode = 2;
} else {
