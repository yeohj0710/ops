...
본문 비교는 빈 줄, 표의 줄바꿈과 열 너비, 내부 페이지 링크와 멘션, 밑줄 이스케이프를 정규화한다.
내부 링크는 같은 페이지 ID여야 한다. 단어, 셀과 순서는 보존한다. 빈칸 자막의 밑줄은 렌더링할 때
이스케이프한다. 별표로 바뀌거나 내용이 빠진 경우는 표시 방식 차이로 통과시키지 않는다.
