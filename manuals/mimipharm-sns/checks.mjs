#!/usr/bin/env node
// 미미팜 인스타그램 운영 준비물 검사. ops done 이 자동으로 돌리고, P0 이 손으로도 돌린다.
//
// 막는 것은 셋뿐이다.
//   1. 드라이브 운영 폴더가 안 보인다 (드라이브가 안 붙었거나 이름이 바뀜)
//   2. 기획안 글판(기획안.md)이 없다
//   3. 하위 폴더(콘텐츠, 편성표, 리포트, 계정 세팅)가 없다 → 만들어 주고 알린다
//
...
const FOLDER = "[운영] 미미팜 인스타그램";
const SUBS = ["콘텐츠", "편성표", "리포트", "계정 세팅"];
...
if (machine?.drive_root) {
  ...
  if (!fs.existsSync(root)) {
    ...
    const shoots = fs
      ...
      .filter((d) => d.isDirectory() && /촬영/.test(d.name))
      .map((d) => d.name);
    알림.push(shoots.length ? `촬영 원본 폴더: ${shoots.join(", ")}` : "촬영 원본 폴더가 없다");
    const plans = fs.readdirSync(path.join(root, "편성표")).filter((f) => /^\d{4}-\d{2}\.md$/.test(f)).sort();
...
console.log("통과. 미미팜 운영 폴더와 기획안이 제자리에 있다.");
