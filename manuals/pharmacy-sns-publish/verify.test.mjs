...
for(const brand of ['aroundpharm','mimipharm']) {
  ...
  for(const alter of [x=>x.actions.pop(),x=>x.actions[0].status='skipped',x=>x.actions.find(a=>a.kind==='repost' && a.actor==='yakdae.saram').status='skipped',x=>x.actions.find(a=>a.status==='skipped').evidence='',x=>x.actions[0].status='blocked',x=>x.actions[0].evidence='',x=>x.actions[1]={...x.actions[0]},x=>x.actions[0].actor='wrong',x=>x.posts={}]) {
