#!/usr/bin/env node
// 반복 실행 한 번마다 인스타에 올릴 후보 한 편을 정한다. P4 앞에 돌린다.
// 여러 편을 요청받으면 한 편의 P6~P8을 끝낸 뒤 이 명령을 다시 실행한다.
//
...
//
// 그래서 규칙을 바꿨다. 반복 실행마다 **가장 오래된 미게시 후보 1편**을 고른다.
// 한 번의 업무에서 몇 편을 올릴지는 사용자 요청이 정하며, 이 스크립트가 한 편으로 제한하지 않는다.
