...
2. **이 저장소를 clone 한다**
3. **`setup.mjs` 를 돌린다** — 기계 등록, 검사 훅, ops 스킬을 Claude·Codex 양쪽에 설치
...
정보와 자격증명 자리도 드라이브에 있다 — `정보/회사.md`·`정보/계정.md` 를 채워두면
에이전트가 서류를 채울 때 안 묻는다. `자격증명/.env` 에 API 키를 두면 새 컴으로 딸려간다.
**비밀번호는 두지 않는다** — 로그인이 필요한 일은 이미 로그인된 크롬(L3)으로 한다.
...
`정보/`·`자격증명/` 은 사람이 채우는 자리라 sync 가 **덮지 않는다** — 있으면 그대로 둔다.

### 드라이브는 발견·배포층, git 은 운영층
