...
const samples=new Set(process.argv.slice(3));
for(const source of await walk(root)){
 ...
 if(item){if(item.bytes!==stat.size||item.mtimeMs!==stat.mtimeMs)throw Error('Source changed: '+rel);if(!samples.has(path.basename(source))||item.backup)continue;}
 ...
 if(samples.has(path.basename(source))){
...
for(const name of ['0318','0403']){await fs.mkdir(path.join(projectBackup,name),{recursive:true});await backupTree(path.join(projects,name),path.join(projectBackup,name));}
const index=path.join(projects,'root_meta_info.json');try{await fs.copyFile(index,path.join(projectBackup,'root_meta_info.json'),constants.COPYFILE_EXCL);}catch(e){if(e.code!=='EEXIST')throw e;}
await fs.writeFile(path.join(work,'project-backup.json'),JSON.stringify(projectManifest,null,2));
