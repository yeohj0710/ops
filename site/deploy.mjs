#!/usr/bin/env node
// 사이트를 굽고 올린 뒤 wbxops.vercel.app 을 그 배포로 돌린다.
//   node site/deploy.mjs
//
// 별칭을 매번 다시 거는 이유: vercel 이 배포마다 새 주소를 준다.
// 프로젝트 SSO 보호는 꺼 뒀다(vercel project protection disable wbxops --sso).
...
const url = (out.match(/https:\/\/wbxops-[a-z0-9]+-[a-z0-9-]+\.vercel\.app/) || [])[0];
if (!url) {
