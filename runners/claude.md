...
1. **같은 일을 하는 다른 도구를 쓴다.** 타이핑이 막히면 폼 채우기 도구가 통한다.
   우회를 노리는 게 아니라 원래 그 일에 쓰는 도구면 된다
2. **사람에게 숫자로 보고하고 승인을 받는다.** 금액, 수량, 대상. 승인이 대화에 있으면
   그 뒤로는 타이핑도 버튼도 통한다. 260818 에 SNS서포터 주문이 이렇게 풀렸다

승인을 받았으면 **그 자리에서 끝까지 누른다.** 화면만 차려 놓고 다음 단계로 넘어가면
